...
public:
	...
	void static createSpecies(std::vector<Species>& speciesVector, sf::Color color, float size, int number);
	void static createCreature(std::vector<Creature>& creatureVector, Species& specie, float x, float y);
	void static moveAllCreature(std::vector<Creature>& creatureVector, sf::Time deltaTime);
private:
